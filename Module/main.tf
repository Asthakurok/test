...
locals {
  ...
  vpc_id = "${element(concat(aws_vpc_ipv4_cidr_block_association.HBI.*.vpc_id, aws_vpc.HBI.*.id, list("")), 0)}"
}

######
# VPC
######
resource "aws_vpc" "HBI" {
...
resource "aws_vpc_ipv4_cidr_block_association" "HBI" {
  count = "${var.create_vpc && length(var.secondary_cidr_blocks) > 0 ? length(var.secondary_cidr_blocks) : 0}"

  vpc_id = "${aws_vpc.HBI.id}"
...
# DHCP Options Set
###################
resource "aws_vpc_dhcp_options" "HBI" {
...
###############################
resource "aws_vpc_dhcp_options_association" "HBI" {
  count = "${var.create_vpc && var.enable_dhcp_options ? 1 : 0}"

  vpc_id          = "${local.vpc_id}"
  dhcp_options_id = "${aws_vpc_dhcp_options.HBI.id}"
}

###################
# Internet Gateway
###################
resource "aws_internet_gateway" "HBI" {
...
resource "aws_route" "public_internet_gateway" {
  ...
  gateway_id             = "${aws_internet_gateway.HBI.id}"
...
#
# but then when count of aws_eip.nat.*.id is zero, HBI would throw a resource not found error on aws_eip.nat.*.id.
locals {
...
resource "aws_nat_gateway" "HBI" {
  ...
  depends_on = ["aws_internet_gateway.HBI"]
}
...
resource "aws_route" "private_nat_gateway" {
  ...
  destination_cidr_block = "0.0.0.0/0"
  nat_gateway_id         = "${element(aws_nat_gateway.HBI.*.id, count.index)}"
...
resource "aws_vpn_gateway_attachment" "HBI" {
...
resource "aws_vpn_gateway_route_propagation" "public" {
  ...
  vpn_gateway_id = "${element(concat(aws_vpn_gateway.HBI.*.id, aws_vpn_gateway_attachment.HBI.*.vpn_gateway_id), count.index)}"
}
